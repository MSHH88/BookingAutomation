...
export async function listSessionsHandler(
  ...
  try {
    ...
    const result   = await svc.listSessions(extractTenantId(req), query);
...
export async function getSessionHandler(
  ...
  try {
    ...
    const session = await svc.getSessionById(extractTenantId(req), id);
...
export async function createSessionHandler(
  ...
  try {
    ...
    const session = await svc.createSession(extractTenantId(req), body);
...
export async function updateSessionHandler(
  ...
  try {
    ...
    const session = await svc.updateSession(extractTenantId(req), id, body);
    res.json(success(session));
  } catch (err) {
    next(err);
  }
...
export async function deleteSessionHandler(
  ...
  try {
    const { id } = req.params as unknown as SessionIdParams;
    await svc.deleteSession(extractTenantId(req), id);
    res.status(204).send();
  } catch (err) {
    next(err);
  }
...
export async function listBookingsHandler(
  ...
  try {
    ...
    const result  = await svc.listSessionBookings(extractTenantId(req), id, page, limit);
    res.json(paginated(result.data, result.meta));
  } catch (err) {
    next(err);
  }
