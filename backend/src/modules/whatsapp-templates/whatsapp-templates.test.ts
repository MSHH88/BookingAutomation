/**
 * Integration tests for /api/messages/whatsapp-templates — Phase 1 (Messaging Foundation)
 *
...
const templateFixture = {
  ...
  tenantId:  'tenant1',
  key:       'lead-inquiry',
  body:      'Hi {{customerName}}, thanks for your inquiry!',
...
describe('GET /api/messages/whatsapp-templates', () => {
  ...
  it('401 when no Authorization header', async () => {
    const res = await request(app).get('/api/messages/whatsapp-templates');
...
describe('GET /api/messages/whatsapp-templates/:key', () => {
  it('200 — returns single template', async () => {
    mockFindFirst.mockResolvedValue(templateFixture);

    const res = await request(app)
      .get('/api/messages/whatsapp-templates/lead-inquiry')
      .set('Authorization', makeToken('ADMIN'));

    expect(res.status).toBe(200);
    expect(res.body.success).toBe(true);
    expect(res.body.data).toMatchObject({ key: 'lead-inquiry' });
  });
  ...
  it('404 when template not found', async () => {
    ...
    const res = await request(app)
      .get('/api/messages/whatsapp-templates/nonexistent')
      .set('Authorization', makeToken('ADMIN'));
...
describe('PATCH /api/messages/whatsapp-templates/:key', () => {
  it('200 — success when ADMIN sends a valid update', async () => {
    ...
    const res = await request(app)
      .patch('/api/messages/whatsapp-templates/lead-inquiry')
      .set('Authorization', makeToken('ADMIN'))
...
describe('POST /api/messages/whatsapp-templates/:key/preview', () => {
  it('200 — returns rendered preview', async () => {
    mockFindFirst.mockResolvedValue(templateFixture);

    const res = await request(app)
      .post('/api/messages/whatsapp-templates/lead-inquiry/preview')
      .set('Authorization', makeToken('ADMIN'))
