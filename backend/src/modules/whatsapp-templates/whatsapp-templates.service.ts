/**
 * WhatsApp Templates service — Phase 1 (Messaging Foundation)
 *
 * Business logic for the WhatsApp Templates admin API.
 *
 * Redis caching strategy:
 *   - Key pattern: `wa_template:{tenantId}:{key}`
 ...
 *   renderTemplate    — load template + interpolate variables (3 tests)
...
import type { ListWhatsAppTemplatesQuery, UpdateWhatsAppTemplateBody } from './whatsapp-templates.schema';

// ─── Cache configuration ──────────────────────────────────────────────────────

const CACHE_PREFIX = 'wa_template';
...
/**
 * Returns a paginated list of WhatsApp templates for a tenant.
 *
...
/**
 * Fetches a single WhatsApp template by tenant + key.
 *
 * Cache strategy:
 *  1. Attempt Redis GET.
...
export async function getTemplateByKey(
  ...
  try {
    ...
    logger.warn('[WhatsAppTemplates] Redis GET failed, falling through to DB', {
  ...
  if (!template) {
    throw new AppError(404, 'TEMPLATE_NOT_FOUND', `WhatsApp template '${key}' not found`);
  }
  ...
  try {
    ...
    logger.warn('[WhatsAppTemplates] Redis SET failed, continuing without cache', {
      error: err instanceof Error ? err.message : String(err),
    });
  }
...
/**
 * Updates an existing WhatsApp template and invalidates the Redis cache.
 *
...
export async function updateTemplate(
  ...
  try {
    ...
    logger.warn('[WhatsAppTemplates] Redis DEL failed after update, cache may be stale', {
      error: err instanceof Error ? err.message : String(err),
    });
  }
...
/**
 * Loads a WhatsApp template by key and interpolates the provided variables.
 *
 * Uses the shared `renderTemplate` from `../../lib/template-renderer` for safe
 * {{variable}} interpolation (no Handlebars helpers/partials).
 *
...
/**
 * Renders a preview of a WhatsApp template with sample variables.
 *
 * Same as `renderTemplate` but explicitly named for the preview endpoint,
...
export async function previewTemplate(
  ...
  const rendered = await renderWhatsAppTemplate(tenantId, key, variables);
  return { key, rendered };
}
